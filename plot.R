#!/usr/bin/env Rscript
qry <- "ht.lajp.fi/users/@me/activity/data?min_duration=1"
...
df <- as.data.frdo.call("rbind", result))
...
ggplot(data, aes(fill = language, y = duration, x = start_time)) +
    ...
    theme(text = element_text(siz", color = "black")) +
...
ggplot(data, aes(fill = project_name, y = duration, x = start_time)) +
    geom_bar(posi242221", color = "black")) +
    labs(x = "Date", y = "Time coded") +
